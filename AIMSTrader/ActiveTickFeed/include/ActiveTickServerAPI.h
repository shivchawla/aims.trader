...
* Welcome to ActiveTick Feed API. This document describes the API and its various support classes responsible for establishing connections to 
* ActiveTick servers, querying static data, and listening for real-time streaming updates.
* @section details API Details
* ActiveTick API utilizes an asynchronous request-response model. All request calls initiated from the API are sent to the server for processing 
* and return immediately with success or fail flag. When responses come back for each call, the API calls an appropriate callback function with the payload for each response.
* It is important to provide all of necessary callback functions to the API, otherwise there would not be any way for API to communicate back to the caller of the functions.
* The API employs a local time-out mechanism for each request function. If data does not arrive in allotted time, the API will call a provided timeout callback function with 
* the original request handle.
*
* @section getstarted Getting Started
* To get started, follow this sequence of calls to initialize the API, connect to the server farm, and authenticate a valid session.
* @li ATInitAPI()
* @li ATCreateSession()
* @li ATSetAPIUserId()
* @li ATSetStreamUpdateCallback()
* @li ATInitSession()
* @li ATCreateLoginRequest() / ATSendRequest()
*
* Once a valid login has been accepted by the server, any of the following functions can be called to request specific data from the server.
...
* @li ATSendRequest()
*
* To cleanup the API, follow this sequence of calls.
* @li ATShutdownSession()
* @li ATDestroySession()
* @li ATShutdownAPI()

* @see ActiveTickServerAPI.h
*/
...
/**
*	Initializes ActiveTick's API. This function must be called before any other can be function called.
...
*	@return UINT64 Number of bytes received since last session initialization.
*/
...
*/
ACTIVETICKSERVERAPI_API bool ATInitSession(uint64_t session, const char* primaryServerHostname, const char* backupServerHostname, uint32_t serverPort, ATSessionStatusChangeCallback pCallback);
...
*	@see ATSessionStatusChangeCallback
*/
ACTIVETICKSERVERAPI_API bool ATInitSession(uint64_t session, const char* primaryServerHostname, const char* backupServerHostname, uint32_t serverPort, ATSessionStatusChangeCallback pCallback, HWND hwndMessagePumpWindow, uint32_t messageId);
...
*	@note Once request handle is created, use SendRequest to send the request to the server. ActiveTick's API guarantees either a notification through ATBarHistoryResponseCallback upon successful data receipt from the server, or a time out notification through ATRequestTimeoutCallback.
*	@see ATSYMBOL
...
*	@see ATSendRequest
*/
ACTIVETICKSERVERAPI_API uint64_t ATCreateBarHistoryDbRequest(uint64_t session, const ATSYMBOL& symbol, ATBarHistoryType barHistoryType, uint8_t intradayMinuteCompression, const ATTIME& beginDateTime, 
																const ATTIME& endDateTime, ATBarHistoryResponseCallback pCallback);
...
*/
ACTIVETICKSERVERAPI_API uint64_t ATCreateBarHistoryDbRequest(uint64_t session, const ATSYMBOL& symbol, ATBarHistoryType barHistoryType, uint8_t intradayMinuteCompression, const ATTIME& beginDateTime, 
																uint32_t recordsWanted, ATCursorType cursorType, ATBarHistoryResponseCallback pCallback);
...
*	@see ATSendRequest
*/
ACTIVETICKSERVERAPI_API uint64_t ATCreateBarHistoryDbRequest(uint64_t session, const ATSYMBOL& symbol, ATBarHistoryType barHistoryType, uint8_t intradayMinuteCompression, uint32_t recordsWanted, 
...
*	@return uint64_t Returns a handle to the request. This handle must be used with ATSendRequest function.
*	@note Once request handle is created, use SendRequest to send the request to the server. ActiveTick's API guarantees either a notification through ATLoginResponseCallback upon successful response from the server, or a time out notification through ATRequestTimeoutCallback.
...
*	@return uint64_t Returns a handle to the request. This handle must be used with ATSendRequest function.
*	@note Once request handle is created, use SendRequest to send the request to the server. ActiveTick's API guarantees either a notification through ATTickHistoryResponseCallback upon successful data receipt from the server, or a time out notification through ATRequestTimeoutCallback.
*	@see ATSYMBOL
*	@see ATTickHistoryResponseCallback
*	@see ATCreateSession
*	@see ATSendRequest
*/
ACTIVETICKSERVERAPI_API uint64_t ATCreateTickHistoryDbRequest(uint64_t session, const ATSYMBOL& symbol, bool selectTrades, bool selectQuotes, const ATTIME& beginDateTime, const ATTIME& endDateTime, 
...
*	@see ATSendRequest
*/
ACTIVETICKSERVERAPI_API uint64_t ATCreateTickHistoryDbRequest(uint64_t session, const ATSYMBOL& symbol, bool selectTrades, bool selectQuotes, const ATTIME& beginDateTime, 
																uint32_t recordsWanted, ATCursorType cursorType, ATTickHistoryResponseCallback pCallback);
...
*	@return uint64_t Returns a handle to the request. This handle must be used with ATSendRequest function.
*	@note Once request handle is created, use SendRequest to send the request to the server. ActiveTick's API guarantees either a notification through ATMarketHolidaysResponseCallback upon successful response from the server, or a time out notification through ATRequestTimeoutCallback.
*	@see ATMarketHolidaysResponseCallback
*	@see ATInitSession
*/
ACTIVETICKSERVERAPI_API uint64_t ATCreateMarketHolidaysRequest(uint64_t session, uint8_t yearsGoingBack, uint8_t yearsGoingForward, ATMarketHolidaysResponseCallback pCallback);
...
*	@note Desired exchange is specified in ATSYMBOL::exchangeType field. ATSYMBOL::symbolType must be set to ATSymbolType::SymbolTopMarketMovers and ATSYMBOL::countryType must be set to ATCountryType::CountryUnitedStates.
*	@note Once request handle is created, use SendRequest to send the request to the server. ActiveTick's API guarantees either a notification through ATMarketMoversDbResponseCallback upon successful response from the server, or a time out notification through ATRequestTimeoutCallback.
*	@see ATSYMBOL
...
*	@li "PL" Percent Losers
*	@note Desired exchange is specified in ATSYMBOL::exchangeType field. ATSYMBOL::symbolType must be set to ATSymbolType::SymbolTopMarketMovers and ATSYMBOL::countryType must be set to ATCountryType::CountryUnitedStates.
*	@note Once request handle is created, use SendRequest to send the request to the server. ActiveTick's API guarantees either a notification through ATMarketMoversStreamResponseCallback upon successful response from the server, or a time out notification through ATRequestTimeoutCallback.
*	@see ATSYMBOL
...
*	@return uint64_t Returns a handle to the request. This handle must be used with ATSendRequest function.
*	@note Once request handle is created, use SendRequest to send the request to the server. ActiveTick's API guarantees either a notification through ATQuoteDbResponseCallback upon successful response from the server, or a time out notification through ATRequestTimeoutCallback.
*	@see ATSYMBOL
...
*	@return uint64_t Returns a handle to the request. This handle must be used with ATSendRequest function.
*	@note Once request handle is created, use SendRequest to send the request to the server. ActiveTick's API guarantees either a notification through ATQuoteStreamResponseCallback upon successful response from the server, or a time out notification through ATRequestTimeoutCallback.
*	@see ATSYMBOL
