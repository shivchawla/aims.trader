/*#include "Platform/View/StrategySpreadPositionView.h"
...
StrategySpreadPositionView::StrategySpreadPositionView(QWidget* parent=0):TreeView<SpreadStrategyPositionView, StrategySpreadPositionViewItem, StrategyPositionModel, StrategyPositionModelColumn>(parent)
{
    setupActions();
...
void StrategyPositionView::addSpread(const StrategyId strategyId, const SpreadId spreadId)
{
      StrategyPositionViewItem* newItem =  _positionMap[strategyId][spreadId];
...
void StrategyPositionView::updatePositionForExecution(const StrategyId strategyId, const TickerId tickerId, const PositionDetail& positionDetail)
{
    if(_positionMap.count(strategyId)!=0)
    {
        if(_positionMap[strategyId].count(tickerId)!=0)
        {
           ...
           item->update((uint)positionDetail.getSharesBought(), StrategyPositionModelBuys);
           item->update((uint)positionDetail.getSharesSold(), StrategyPositionModelSells);
           item->update((uint)positionDetail.getNetShares(), StrategyPositionModelNet);
...
void StrategyPositionView::updatePositionForLastPrice(const StrategyId strategyId, const SpreadId spreadId, const double runningPnl, const double pnl)
{
    if(_positionMap.count(strategyId)!=0)
    {
        if(_positionMap[strategyId].count(tickerId)!=0)
...
void StrategyPositionView::updatePositionForLastPrice(const StrategyId strategyId, const SpreadId spreadId, const TickerId tickerId, const double runningPnl, const double pnl)
{
    if(_positionMap.count(strategyId)!=0)
    {
        if(_positionMap[strategyId].count(tickerId)!=0)
...
void StrategyPositionView::removePosition(const StrategyId strategyId, const SpreadId spreadId)
{
